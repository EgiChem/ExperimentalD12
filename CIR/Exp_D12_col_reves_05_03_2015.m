function Exp_D12_col_reves_05_03_2015()

clear all 
clc
format short

global R_col L_col Dados_Experimentais 
...
[data_matrix, text_data, alldata] = xlsread(...
    'D12_180503_120314_EtOH_CO2_175bar_40ºC_coluna3.xlsx',3); 
...
for j1 = 1:posicao_max_absorbance
    if Dados_Experimentais.absorbance(j1) == 0.1*max_absorbance 
...
if n1 ==0 %se não existir nenhum ponto correspondente a 0.1*maxabs
    dif_absorb1 = abs(Dados_Experimentais.absorbance(1:posicao_max_absorbance)- 0.10*max_absorbance); %diferença entre cada ponto e o maxabs
...
% ---
    ...
    if absorbance_1 < 0.1*max_absorbance % se o ponto + proximo esta abaixo
    ...
    elseif absorbance_1 > 0.1*max_absorbance % se o ponto + proximo esta acima
    ...
    S10_1_time = polyval(p_1,0.1*max_absorbance); %tempo correspondente a 0.1*maxabs
    
    tempo_I_ver = polyval(p_1,linspace(min(absorbance_interpolar_1),max(absorbance_interpolar_1), 500)); %para ver no grafico
    
    hold on
    figure(1)
    plot(tempo_intepolar_1, absorbance_interpolar_1, '*r', tempo_I_ver, linspace(min(absorbance_interpolar_1),max(absorbance_interpolar_1), 500), '-r', S10_1_time, 0.1*max_absorbance, 'ob') %representação do 0.1*maxabs, do ponto exp + proximo e da f. interpolação
...
for j2 = posicao_max_absorbance+1:length(Dados_Experimentais.tempo)
    if Dados_Experimentais.absorbance(j2) == 0.1*max_absorbance 
...
if n2 ==0
    dif_absorb2 = abs(Dados_Experimentais.absorbance(posicao_max_absorbance+1:length(Dados_Experimentais.tempo))- 0.10*max_absorbance);
    ...
    if absorbance_2 < 0.1*max_absorbance
    ...
    elseif absorbance_2 > 0.1*max_absorbance
    ...
    S10_2_time = polyval(p_2,0.1*max_absorbance);
    
    tempo_II_ver = polyval(p_2,linspace(min(absorbance_interpolar_2),max(absorbance_interpolar_2), 500));
    
    hold on
    figure(1)
    plot(tempo_intepolar_2, absorbance_interpolar_2, '*r', tempo_II_ver, linspace(min(absorbance_interpolar_2),max(absorbance_interpolar_2), 500),'-r', S10_2_time, 0.1*max_absorbance, 'ob')
    xlabel('{\itt} (s)')
    ylabel('Absorbance (AU)')
end    
% ---    
...
%% - Resultados a partir do cálculo da velocidade linear a partir das condições de operaçõ da BOMBA 
...
Resultados_CIR.S10 = S10;
Resultados_CIR.u0_exp = u0_exp; % (cm/s) velocidade linear dentro da coluna - calculadas a partir das condiçoes operatorias da bomba
Resultados_CIR.max_absorbance=max_absorbance;
Resultados_CIR.D12 = D12_optim_CIR; % (cm^2/s) Valor da difusividade experimental
Resultados_CIR.k_optim = k_optim_CIR_1; % coeficiente de retenção 
Resultados_CIR.epsilon = epsilon*100;
...
elseif aviso1 == 1 &&  aviso2 ~= 1
    ...
    absorbance_integral_final = [absorbance_integral 0.10*max(Dados_Experimentais.absorbance)];
    time_integral_final = [time_integral t2];
        
elseif aviso1 ~= 1 &&  aviso2 == 1
    
    % Pontos necessários para calcular o integral entre os valores t1 e t2   
    absorbance_integral_final = [0.10*max(Dados_Experimentais.absorbance) absorbance_integral];
    time_integral_final = [t1 time_integral];
        
elseif aviso1 ~= 1 &&  aviso2 ~= 1
    
    % Pontos necessários para calcular o integral entre os valores t1 e t2
   absorbance_integral_final = [0.10*max(Dados_Experimentais.absorbance) absorbance_integral 0.10*max(Dados_Experimentais.absorbance)];
...
Ca_norm_num_alldata = 1./(4*pi*a_optim.*tempo_detalhado).^(1/2).*exp(-(L_col-U_optim.*tempo_detalhado).^2./(4.*a_optim.*tempo_detalhado)); % g/cm3 -> pela expressão (Eq. 25) dada no artigo Kong et al (J. Chromatogr. A, 1035, 177)  
...
Resultados_CIR.Area_Absorbance = Area_Absorbance; 
Resultados_CIR
Fim = '';
...
global R_col L_col Dados_Experimentais m_real_inject
...
if Metodo_Integral == 1
    ...
    % Área da expressão anterior - integral da expressão anterior em ordem ao tempo.
    Area_Ca_norm_denom = trapz(intervalo_t,Ca_norm_num_trap); % (s.cm^(-1)) -> Método dos trapézios
