function D12_modelacao_04_06_2013_U_a_CIR()
...
[data_matrix, text_data, alldata] = xlsread(...
    '2015_03_04fenol_acetona_tolueno_etanol_em_CO2_250bar_40ºC_coluna2.xlsx',8); 
...
u0_exp_2 = Q_estufa_2/60/Area_SR_coluna; %cm/s
...
if Metodo_Integral == 1
    ...
    % Normalização da curva calculada [(normalização segundo a expressão (Eq. 24) que é fornecido no artigo Kong et al. (J. Chromatogr. A, 1035, 177)]  
%     Ca_norm_num = 1./(4*pi*a.*time_integral_final).^(1/2).*exp(-(L_col-U.*time_integral_final).^2./(4*a.*time_integral_final)); % cm^(-1)
